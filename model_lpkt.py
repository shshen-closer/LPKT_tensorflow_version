...
class LPKT(object):
    ...
    def __init__(self, batch_size, num_steps, num_skills, hidden_size):
        ...
        # exercise embedding
        self.problem_w = tf.Variable(tf.compat.v1.keras.initializers.VarianceScaling()([53091, hidden_size]),dtype=tf.float32, trainable=True, name = 'problem_w')
        ...
        #answer time embedding
        self.at_w = tf.Variable(tf.compat.v1.keras.initializers.VarianceScaling()([26747, hidden_size]),dtype=tf.float32, trainable=True, name = 'at_w')
        ...
        #KC matrix
        kc_matrix = tf.Variable(tf.compat.v1.random_uniform([53091, num_skills], minval=0.0, maxval=0.1,dtype=tf.float32), trainable=True, name="kc_matrix")
        zero_kc = tf.zeros((1, num_skills))
        all_kc = tf.concat([zero_kc, kc_matrix], axis = 0)
        
        ikc = tf.cast(self.input_kc, tf.int32)
        tkc = tf.cast(self.target_kc, tf.int32)
        ikc_embedding =  tf.nn.embedding_lookup(all_kc, self.input_problem)
        tkc_embedding =  tf.nn.embedding_lookup(all_kc, self.target_id)
        ...
        # forgetting vector 
        self.it_w = tf.Variable(tf.compat.v1.keras.initializers.VarianceScaling()([29748, hidden_size]),dtype=tf.float32, trainable=True, name = 'it_w')
        ...
        slice_it_embedding = tf.split(it_embedding, self.num_steps+2, 1)

        shape = at_embedding.get_shape().as_list()
        padd = tf.zeros((shape[0], 2, shape[2]))
        at_embedding = tf.concat([padd, at_embedding], axis = 1)
        slice_at_embedding = tf.split(at_embedding, self.num_steps+2, 1)
        ...
        for i in range(2,self.num_steps+2):
            ...
            it_zero = tf.squeeze(slice_it_embedding[i-1], 1)

            kc_one = slice_input_kc[i]

            at_one = tf.squeeze(slice_at_embedding[i], 1)
            at_zero = tf.squeeze(slice_at_embedding[i-1], 1)
